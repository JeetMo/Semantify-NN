...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 30 21:21:37 2019
...
from tensorflow.python.keras.layers import Dense, Dropout, Activation, Flatten, GlobalAveragePooling2D, Lambda
from tensorflow.python.keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, InputLayer, BatchNormalization, Reshape
...
class NLayerModel:
    def __init__(self, params, restore = None, session=None, use_log=False, image_size=28, image_channel=1, activation='relu', num_labels=10):
...
def loss(correct, predicted):
        return tf.nn.softmax_cross_entropy_with_logits(labels=correct,
                                                       logits=predicted)
##from cnncert.train_resnet import ResidualStart, ResidualStart2

class cnnModel:
    def __init__(self, file_name, inp_shape = (28,28,1)):
        model = load_model(file_name, custom_objects={'fn':loss, 'ResidualStart':ResidualStart, 'ResidualStart2':ResidualStart2, 'tf':tf})
...
class CNNModel:
    def __init__(self, file_name, inp_shape = (28,28,1)):
        ...
        model = load_model(file_name, custom_objects={'fn':loss, 'tf':tf})
        #model = load_model(file_name, custom_objects={'fn':loss, 'ResidualStart':ResidualStart, 'ResidualStart2':ReidualStart2, 'tf':tf})
